...
**執行時發現的版本衝突(實機驗證後修正)**
- `@prisma/instrumentation@5.x`(對應 Prisma 5)是針對 **OTel SDK 1.x** 寫的,與本專案的 **OTel SDK 2.x**(`sdk-node@0.218`)不相容 —— 掛上 ESM hook 後會在第一筆查詢 crash(`parentTracer.getActiveSpanProcessor is not a function`)。已**移除 `@prisma/instrumentation` + ESM hook**。
- DB 觀測改用**版本無關**的做法:`createPrismaClient()`(`shared/db/prisma.ts`)以 `log:[{emit:'event',level:'query'}]` + `$on('query')` 把每筆查詢時間記入 `im_db_query_duration` histogram,並對慢查詢(`DB_SLOW_QUERY_MS`,預設 100ms)發 log(**不記 params**,避免洩漏 PII)。四個 factory 的 default client 改走此 instrumented factory(仍各自一個 client,**不是**共享單例)。
- 對「找 DB 瓶頸」而言,query duration 的 p95/p99 分佈其實比單筆 SQL span 更直接可用。
...
> 完整的「從零啟動 → 製造流量 → 在 Grafana 輸入什麼查詢看什麼 → 跑壓測找瓶頸」操作手冊見
> [observability/QUICKSTART.md](observability/QUICKSTART.md)；訊號對照表見 [observability/SIGNALS.md](observability/SIGNALS.md)。

> 在 GitHub Codespaces 上,`.devcontainer/devcontainer.json` 已用 docker-in-docker
> 並在 postCreate 自動建好 `obs-net` 網路、forward 好 3000/8080/8081/4040,可直接跳到第 1 步。

```bash
# 0) 一次性：建共用網路（Codespaces 已自動建好，本機 Podman 才需要）
...
**驗收清單（Phase E）**
- [ ] Tempo 看得到 HTTP server span（GET/POST `/api/v1/...`）
- [ ] DB 觀測改走 metric：Mimir 有 `im_db_query_duration`（p50/p95/p99）+ 慢查詢進 Loki（`@prisma/instrumentation` 與 OTel SDK 2.x 不相容，已改用 Prisma `query` event，見下方 deltas）
