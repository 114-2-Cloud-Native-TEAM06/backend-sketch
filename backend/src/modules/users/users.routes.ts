import { Router, Request, Response } from 'express';
import { PrismaClient } from '@prisma/client';
import authMiddleware from '../shared/auth/auth-middleware.js';
import { createPrismaClient } from '../shared/db/prisma.js';
import { getMe, getUserById, updateMe } from './users.service.js';

export function createUserRouter(prisma: PrismaClient = createPrismaClient()): Router {
  const router = Router();

  router.get('/me', authMiddleware, async (req: Request, res: Response): Promise<void> => {
    const user = await getMe(prisma, req.user!.userId);
    res.json(user);
  });

  router.patch('/me', authMiddleware, async (req: Request, res: Response): Promise<void> => {
    const user = await updateMe(prisma, req.user!.userId, req.body);
    res.json(user);
  });

  router.get('/:id', authMiddleware, async (req: Request, res: Response): Promise<void> => {
    const user = await getUserById(prisma, req.params.id);
    res.json(user);
  });

  return router;
}
